...
void Game::initialize()
{
	...
	ifstream inFile;
	inFile.open("tshader.txt");
	std::string line;
	std::stringstream finalOutput;
	finalOutput << inFile.rdbuf();
	line = finalOutput.str();

	/* Vertex Shader which would normally be loaded from an external file */
	const char* vs_src = line.c_str(); //Vertex Shader Src
	inFile.close();
	...
	inFile.open("fshader.txt");
	line = "";
	finalOutput.str(std::string());
	finalOutput << inFile.rdbuf();
	std::cout << finalOutput.rdbuf() << std::endl << std::endl;
	line = finalOutput.str();
...
void Game::update()
{
	...
	for (int i = 0; i < 8; i++)
	{
		MyVector3 tempVector(vertex[i].coordinate[0], vertex[i].coordinate[1], vertex[i].coordinate[2] + 0.25);
		tempVector = mCaller.RotateAboutX(tempVector, 0.1);
		tempVector = mCaller.RotateAboutY(tempVector, 0.1);
		vertex[i].coordinate[0] = tempVector.GetX();
		vertex[i].coordinate[1] = tempVector.GetY();
		vertex[i].coordinate[2] = (tempVector.GetZ() - 0.25);
	}
